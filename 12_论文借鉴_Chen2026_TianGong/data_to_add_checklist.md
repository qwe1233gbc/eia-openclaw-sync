...
- QA 候选池：最新主工作簿已形成 1037 行候选 QA，并补充 95 条样本链 QA；
...
| P0 | 从 95 条样本链 QA 中筛选出的正式评价候选题 | 已有 60 题，需形成 40-60 道 verified gold | A/B/C/D 对照实验 | 现有 QA 仍需人工金标复核 | `outputs/next_execution_plan_20260706/` |
| P1 | 标准依据库人工校准记录 | 先校准正式候选题涉及条目 | 提升 RAG 和 skill 依据可靠性 | 表号、限值、适用条件需复核 | `03_指南解析_明文标准库/quality/` |
...
| P2 | A/B/C/D 四组实验输出 | 40-60 道 verified gold x 4 组 | 对照实验分析 | 尚无正式输出矩阵 | `outputs/next_execution_plan_20260706/` |
| P2 | 人工评分记录 | 至少 2 名评分者，40-60 道 verified gold | 统计分析和一致性检验 | 缺少人工评分 | `outputs/next_execution_plan_20260706/` |
...
当前 1037 行候选 QA 和 95 条样本链 QA 只能作为候选池，不能直接作为正式测试集。

建议先整理一版小规模正式评价题：

- 数量：优先复核 10-15 道样例，后续扩展到 40-60 道 verified gold；
- 来源：优先来自完整样本链和真实修改意见；
...
建议至少 2 名评分者，先评分 40-60 道 verified gold。每条记录包括：
...
3. 从 60 道正式评价候选题中优先复核 10-15 道样例，并扩展到 40-60 道 verified gold；
4. 补齐每题的报告证据、标准依据和真实修改意见；
5. 建立 rejected log。

### 第二批：补能让结果可信的数据

1. 优先校准 60 道候选题涉及的标准卡或标准条目；
2. 用真实样本校准 10 个优先 skill；
3. 完成 40-60 道 verified gold 的 A/B/C/D 输出；
4. 完成至少 2 名评分者的人工评分；
5. 汇总错误类型和评分分歧。
...
2. **QA 题库清理**：我来统一 1037 行候选 QA、95 条样本链 QA 和 60 道候选题的字段名与复核状态。
3. **正式评价题复核**：我从 60 道候选题中优先复核 10-15 道样例，并扩展到 40-60 道 verified gold。
...
> 先对 `05_样本链（按项目组织）/` 做 readiness 盘点，并从 60 道正式评价候选题中复核出第一批 verified gold。
