...
def main() -> None:
    ...
    (ROOT / "03_formal_chunks" / "chunk_quality_report.md").write_text(
        f"# 正式RAG分块质量报告\n\n- 可入库来源：{sum(bool(s['eligible_for_formal_rag']) for s in sources)}\n"
        f"- 父块：{len(parents)}\n- 子块：{len(children)}\n"
        f"- 分块策略：法规/标准优先按标题、条款、PDF页标记切分；父块目标约1000–3000字符，子块约850–1000字符并保留150字符重叠。\n"
        f"- 表格与完整条款：未按固定token强制截断；PDF抽取仍应在关键表格处人工核图。\n"
        f"- 排除：缺失全文来源、仅元数据来源、QA、Skill、人工答案和实验输出均未入库。\n",
        encoding="utf-8")
    ...
    for task in tasks:
        ...
        if req["current_status"] == "not_required":
            ...
            # Score the full whitelist so every declared required source can be
            # deterministically considered by the fixed source-priority step.
            # Only the top 40 are persisted as ordinary sparse candidates.
        ...
        snap = {
            ...
            "rag_context_sha256": context_hash, "retrieval_mode": "provisional_bm25_plus_fixed_source_rerank",
            "formal_hybrid_rag": False, "embedding_model": "not_configured", "embedding_revision": "not_configured",
            "reranker_model": "fixed_source_priority_not_neural", "reranker_revision": "v1",
            "candidate_k_sparse": 40, "candidate_k_dense": 0, "rerank_k": 8, "final_parent_k": 8,
    ...
    # Phase 2 starts only after the frozen snapshot and marker have been written.
...
当前不能运行21题正式A/B/C/D实验，原因是仍有权威原文缺失，且Dense Embedding与神经Reranker尚未配置。B/D同题RAG哈希一致率为{sum(x['hash_equal'] for x in hashes)}/{len(hashes)}（100%），QA泄漏={isolation['qa_leakage_count']}，Skill程序泄漏={isolation['skill_procedure_count']}。
...
"""
    ...
    (ROOT / "07_code" / "requirements.txt").write_text("openpyxl>=3.1\npypdf>=5.0\n", encoding="utf-8")
    ...
    summary = {
        "counts": dict(counts), "source_count": len(sources), "eligible_source_count": sum(bool(s["eligible_for_formal_rag"]) for s in sources),
        ...
        "can_start_formal_abcd": counts["incomplete"] == 0 and isolation["isolation_pass"] and all(x["hash_equal"] for x in hashes) and False,
        "blocking_issues": ["incomplete题目仍存在", "Dense Embedding未配置", "神经Reranker未配置"],
    }
