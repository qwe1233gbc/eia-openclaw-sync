...
> 更新时间：2026-07-07  
> 当前结论：本课题学习 Chen 论文的数据组织、检索增强、工作流化和专家验证思路，但不做微调；当前项目已有标准依据库、审核技能库、样本链证据库、95 条样本链 QA、60 道正式评价候选题和 A/B/C/D 实验运行表，下一步重点是人工金标复核。
...
- `project_data_flow_against_chen2026.md`：按当前项目真实数据流，说明哪些数据可用、哪些不可用，以及不做微调时如何迁移 Chen 的方法骨架。
- `outputs/data_audit_against_chen2026_20260707/data_audit_usable_vs_unusable.md`：最新数据审核结论，建议作为当前数据口径依据。

## 1.1 当前项目与 Chen 的最新对应关系

| Chen 论文思路 | 本课题对应数据 | 当前状态 |
|---|---|---|
| 权威环境材料结构化 | 佛山塑胶指南、标准卡、Dify 候选条目、v4 skill 对齐标准条目 | 已形成基础，需人工校准 |
| 检索增强 | 标准依据库、cards_by_skill、混合检索配置 | 可作为 B/D 组输入 |
| 工作流化任务执行 | 15 个单项审核 skill + 1 个总审核 skill | 可作为 C/D 组输入 |
| 生成候选任务 | 原始 942 道 QA、合并后主表 1037 行、95 条样本链 QA | 可作为候选池 |
| 专家/人工验证 | 全海世家专家评分表、专家意见映射、gold 复核模板 | 已有试点，仍需扩展 |
| 对照评价 | 60 道正式候选题、240 行 A/B/C/D 运行表 | 设计完成，等待金标复核 |

需要明确：Chen 的 TianGong 数据集不能作为本课题的环评审核金标；本课题只借鉴其“数据流和验证方法”，不迁移其微调路线。
